...
//scrollable on ipad
		if(($(window).scrollTop() + $('.page-header').outerHeight()) >= $('.scores-section').offset().top ) {
	...
	if($(window).width() >= 768 && $(window).width() <= 1024) {
		var sectionHeaderTop = ($(window).height()
								- headerHeight
		...
		//$('.scores-section .section-header').css({'top': sectionHeaderTop + 'px'});
